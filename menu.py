...
class Menu:
    ...
    def pause_menu(self):
        ...
        """
        pause_menu_figures = {
            'resume': pygame.transform.scale(pygame.image.load('data/PNG/Retina/Effects/explosion2.png'), (350, 60)),
            'help': pygame.transform.scale(pygame.image.load('data/PNG/Retina/Effects/explosion2.png'), (350, 60)),
        ...
        while True:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    sys.exit()
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if 380 <= event.pos[0] <= 620 and 250 <= event.pos[1] <= 290:
                        screen.blit(self.background,(0,0))
                        pygame.display.update()
                        if os.path.isfile('data/Audio/background.mp3'):
                        ...
                        self.help_menu()
                    elif 415 <= event.pos[0] <= 585 and 530 <= event.pos[1] <= 570:
                        pygame.quit()
                        sys.exit()
            ...
            if resume_rect.collidepoint(pygame.mouse.get_pos()):
                screen.blit(pause_menu_figures['resume'], (resume_rect.x - 60, resume_rect.y - 8))
            screen.blit(resume, resume_rect)

            help_rect = help.get_rect(center=(WIDTH // 2, 400))
            if help_rect.collidepoint(pygame.mouse.get_pos()):
                screen.blit(pause_menu_figures['help'], (help_rect.x - 55, help_rect.y - 8))
            screen.blit(help, help_rect)

            exit_rect = exit.get_rect(center=(WIDTH // 2, 550))
            if exit_rect.collidepoint(pygame.mouse.get_pos()):
                screen.blit(pause_menu_figures['exit'], (exit_rect.x - 60, exit_rect.y - 6))
